...
| `acceleratedStep_equiv_standardStep` | Checked | `CollatzResearch.Equivalence` | One accelerated step on odd domain = 1 + ν₂(3n+1) standard steps (closed in PR #46 at `f112f40`+1; 4-`rw` composition of `standardTrajectory_succ_shift` (PR #42) + `standardStep_of_odd` (PR #43) + `standardTrajectory_pow_div` (PR #45); divisibility witness via `Prime.pow_dvd_iff_le_factorization (Nat.prime_two) (by positivity) |>.mpr (le_refl _)`; final `rw [acceleratedStep, twoAdicValuation]` closes by `rfl`) |
