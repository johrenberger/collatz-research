...
| `standardTrajectory_pow_div` | Pending | `CollatzResearch.Equivalence` | Power division: `standardTrajectory x k = x / 2^k` when `2^k ∣ x`; library-light; foundation for `acceleratedStep_equiv_standardStep` (proof TODO; submitted via branch `story-02c-03c-equivalence-helper-1`, awaiting CI) |
