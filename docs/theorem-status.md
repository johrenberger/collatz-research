...
| `coverage_tree_soundness_full` | Checked | `CollatzResearch.CoverageTree` | Semantic-leaf soundness: under `ValidTree t ∧ IsComplete t` + per-leaf `LeafReachesOne` hypothesis `hLeaf`, every `0 < x` descends to a leaf `l` with `LeafReachesOne t l`. Closed in PR #49; proof is `obtain ⟨l, hl, hver, hdesc⟩ := coverage_tree_soundness t hv hic x hx; exact ⟨l, hl, hver, hdesc, hLeaf l hl hver hdesc⟩` — no new `sorry`/`admit`/`axiom`. Conditional form (re-scoped from unconditional v1 per Codex P0) avoids claiming `ReachesOne x` for every `x > 0` under any valid complete tree (which would amount to the Collatz theorem). |
