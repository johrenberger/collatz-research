...
theorem checkBoundedCertificate_sound
  ...
  sorry  -- Q5 PR #2 (verifier) — proof deferred to integration PR
  -- Constructs `BoundedInputOrbitCertificate t l N` from the verified witnesses.
  -- The `orbit_hits_claim` field uses each witness's trajectory to find `k`
  -- such that `claim.Holds (accelerated_orbit w.x k)`.
  -- The `claim_reaches_one` field uses the trajectory's final value (which
  -- `checkTrajectory` verified satisfies `claim.Holds`) and the
  -- `accelerated_orbit` dynamics to show `ReachesOne y`.
